...
| `taifex_fetch.py` | 期交所 mis API + 歷史回補；到期別代碼解析(`parse_expiry_code` / `ContractMonth`)；剩餘時間T(`years_to_settlement`，錨在到期日09:30結算) |
...
8. **下面這幾個地方是踩過坑修好的，看起來多餘但不要「簡化」回去**：
   ...
   - `db.get_baseline_iv_batch()` 的 `trade_date >=` 下限 + `pricing_service` 的
     ...
     只留一道會漏掉另一種。
   - `taifex_fetch.years_to_settlement()` **可以回傳 0 或負數**，不要在外面包
     `max(T, 下限)` 把它變正 —— 那正是 B 原本的錯(`max(days,1)/365`)，
     會讓結算後的合約擠出一個不存在的IV，畫面還照常上色。
     結算後要走 `ContractSettledError`，不是硬算。
   - `pricing_service.pick_contract()` 會跳過已結算的到期別。期交所到期後不會
     馬上下架(實測 2026-09-12 清單第一個就是前一天到期的 202609F2)，
     不跳過的話畫面預設就是一片錯誤訊息。使用者自己指定的不擋。
...
**2026-09-12 已修掉 A、B、C —— 三個紅的全部修完**，剩 5 個黃/綠的，
見下面「下次要做：修剩下這 5 個邏輯問題」。
離線自我測試那 5 支當時全過 —— 那些問題測試測不到，所以修的時候都補了對應斷言。
...
## 下次要做：修剩下這 5 個邏輯問題 (2026-09-11 全面檢查找到的)

離線自我測試那 5 支當時全過，這些是「測試測不到、但會實際誤標」的問題。
**A / B / C 三個紅的已於 2026-09-12 修完**(保留在下面當紀錄)，
剩下 D~H 是黃綠的，其中 D 跟 E 都是「期交所API沒給日期/時戳」造成的，修不乾淨。
...
### B. ~~到期日當天 `T` 被灌水成 1/365~~ ✅ 已修 (2026-09-12)

**原本的問題**：`T = max(days_to_expiry, 1) / 365`，到期日 `days=0` → `T=1/365`，
但實際只剩幾小時。同鏈基準會自我抵銷(反推跟定價用同一個錯的T)，所以偏斜曲線/ATM
那兩條路判斷結果不受影響；但 **DB 5日均那條完全不抵銷**，因為IV是與T無關的參數：

```
到期日 08:30、市場真實 IV=0.20
  舊 T = 0.002740 年(24.0小時)  反推出來的 IV = 0.0408   ← 畫面顯示的IV本身就是錯的
  新 T = 0.000114 年( 1.0小時)  反推出來的 IV = 0.2000   ✅
  基準=DB 5日均(也是0.20)：修正前整條鏈 cheap -67.7%，修正後 偏貴0/合理14/便宜0
```

**決定的答案：T 算到到期日 09:30 歸零**(`taifex_fetch.SETTLEMENT_TIME`)。
TXO 最後結算價 = 到期日開盤後30分鐘內加權指數的簡單算術平均(09:00–09:30)，
09:30 一到損益就定了 —— 它還會掛牌交易到 13:45，但那之後的價格沒有時間價值。

**修法**：

- `taifex_fetch.years_to_settlement(expiry_date, now)` —— 用真實時刻算，
  回傳「年」。放在 `taifex_fetch` 是因為這是期交所的規則，而且
  `pricing_service` 跟回補歷史都要用它(放 pricing_service 會變成循環 import)。
- **它可以回傳 0 或負數**，而且刻意不給下限。結算後 `analyze_chain()` 丟
  `ContractSettledError`(繼承 `NoQuoteDataError`，所以 CLI 跟網頁後端原本的
  `except` 就接得住 —— CLI 印訊息收工、網頁只標這一邊算不出來，另一邊照常顯示)。
- `pick_contract()` 預設會跳過已結算的到期別(見硬規則8)。
- `backfill_monthly_iv_history()` 也改用同一支算 T(錨在那天的收盤時刻：
  日盤 13:45 / 夜盤 05:00，`SESSION_CLOSE_TIME`)。兩邊T基準不一致的話，
  寫進 `option_iv_history` 的IV會系統性偏掉 —— 而那是最高優先序的基準。
- 順帶：一般日子的 T 現在也含小時了(到期前4天，09:00 算 4.02 天、23:00 算 3.44 天)，
  不再是整數天。同鏈會抵銷所以判斷幾乎不變，但畫面顯示的IV更接近真的。
- 測試：`taifex_fetch.py` Part 1(離線那段)加了 T 的斷言 ——
  到期日 08:30 剩1小時、09:30 整歸零、之後為負、夜盤凌晨3點剩6.5小時。

**沒動的地方**：`mock_live_quotes.py` / `run_demo_pipeline.py` 還是用 `days/365`。
那兩支是離線 demo，產生價格跟反推IV用的是同一個T，自我一致，換過去沒有意義。
